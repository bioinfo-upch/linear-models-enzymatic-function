...
# Creating a new variable that indicates if kcat is greater than the median
...
data.nm.k.2 <- mutate(.data=data.nm.k.2,Kcat=as.factor(ifelse(data.nm.k[,1]>=median.kcat,
                                                              paste('\u2265 ',median.kcat,sep=''),
                                                              paste('< ',median.kcat,sep=''))))
data.nm.k.1 <- filter(data.nm.k, Kcat<median.kcat)
data.nm.k.1 <- mutate(.data=data.nm.k.1,Kcat=as.factor(ifelse(data.nm.k[,1]>=median.kcat,
                                                              paste('\u2265 ',median.kcat,sep=''),
                                                              paste('< ',median.kcat,sep=''))))
...
test.table <-  cbind(position,conf.int.lower,conf.int.upper,p.values)

...
gg.intervals <- function(model,variables,intercept=T){
  ...
  ggplot(td, aes(estimate, Parameters, color=Parameters)) +
    ...
    ylab('Coefficients') + xlab('95% Confidence Intervals') 
}
...
plot.scatter.model <- function(model,kinetic_label){
  ...
  pred <- predict(model)
  aug <- cbind(aug,pred)
  lims_max <- ceiling(max(c(model$model[,1],model$fitted.values))) 
  lims_min <-  floor(min(c(model$model[,1],model$fitted.values)))
  ggplot(aug, aes_string(x=kinetic_parameter,y='.fitted',label='rownames(aug)')) + 
    geom_point() + ylim(lims_min,lims_max) + xlim(lims_min,lims_max) + geom_abline(col=2) +
    ylab(bquote("Predicted <"~log[10]~"( Relative "~.(kinetic_label[[1]])~') >')) +
    xlab(bquote("<"~log[10]~"( Relative "~.(kinetic_label[[1]])~') >')) + geom_text(hjust=-0.25,size=2.5) +
    theme(axis.line = element_line(colour = 'black'), panel.background = element_blank(),
          panel.grid.major = element_blank(), panel.grid.minor = element_blank())
}
...
cv.rmse.density <- function(..., model.name, variable){
  ...
  plot <- ggplot(dat, aes(x=RMSE, fill=Model)) + geom_density(alpha=0.5) +
    theme(axis.title = element_text(size = 10), axis.text = element_text(20),
          ...
          panel.grid.major=element_blank(), panel.grid.minor=element_blank()) +
    labs(x=bquote("RMSE distribution of predicted <"~log[10]~"( Relative "~.(variable[[1]])~') >'),
         y='Density')
  return(plot)
}                
